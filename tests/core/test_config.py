...
def test_settings_env(root_dir_path: Path):
  ...
  assert bool(strtobool(env_values["SQL_LOGGING"])) == config.get_settings().SQL_LOGGING

  # 認証コード設定
  assert int(env_values["AUTHCODE_LENGTH"]) == config.get_settings().AUTHCODE_LENGTH
  assert int(env_values["AUTHCODE_EXPIRE_MINUTES"]) == config.get_settings().AUTHCODE_EXPIRE_MINUTES
